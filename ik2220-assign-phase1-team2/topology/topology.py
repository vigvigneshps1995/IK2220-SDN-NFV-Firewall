    ...
    with  open('../report/report.txt','w') as f:
        ...
        for PbZ_h in ["h1", "h2"]:
            ...
            for DmZ_s in ['100.0.0.40','100.0.0.41','100.0.0.42']:
                ...
                std = str(PbZ_h.cmd('curl --connect-timeout 0.1 {}:80 -I'.format(DmZ_s)))
                log = str(PbZ_h.cmd('curl --connect-timeout 0.1 {}:80 -I'.format(DmZ_s)))
                ...
                for a in range(10):
                    ...
                    log = str(PbZ_h.cmd('curl --connect-timeout 0.1 {}:{} -I'.format(DmZ_s,port)))
                    f.write( succeed() if log != std else fail())
        ...
        for PrZ_h in ["h3", "h4"]:
            ...
            for DmZ_s in ['100.0.0.40','100.0.0.41','100.0.0.42']:
                ...
                std = str(PrZ_h.cmd('curl --connect-timeout 0.1 {}:80 -I'.format(DmZ_s)))
                log = str(PrZ_h.cmd('curl --connect-timeout 0.1 {}:80 -I'.format(DmZ_s)))
                f.write( succeed() if log == std else fail())
                
                for a in range(10):
                    port = random.randint(1,10000)
                    f.write('[Test2]From PrZ: {} ping DmZ: {} random port = {}: it should not work!\n'.format(PrZ_h, DmZ_s,port))
                    log = str(PrZ_h.cmd('curl --connect-timeout 0.1 {}:{} -I'.format(DmZ_s,port)))
                    f.write( succeed() if log != std else fail())
